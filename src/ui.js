...
export function setupUI(sim) {
    ...
    // ─── Info tips ───
    const infoData = {
        energy: { title: 'Energy Conservation', body: 'Total energy = Linear KE + Spin KE + Potential + Field + Radiated. Field is the Darwin Lagrangian velocity-dependent EM field energy. Radiated tracks cumulative energy lost to Larmor radiation. Drift indicates numerical integration error. Spin KE uses I = (2/5)mr\u00B2 (uniform-density solid sphere).' },
        conserved: { title: 'Conserved Quantities', body: 'Momentum = |p_particle + p_field + p_radiated| (vector sum). Particle momentum is \u03A3(m\u1D62w\u1D62). Field momentum is the Darwin EM momentum from charged particle pairs. Radiated momentum accumulates from Larmor radiation recoil. Angular momentum about the center of mass splits into orbital \u03A3(r\u1D62 \u00D7 m\u1D62w\u1D62) and spin \u03A3(I\u1D62S\u1D62) where I = (2/5)mr\u00B2. Conserved with gravity and Coulomb only. Velocity-dependent forces (Lorentz, linear gravitomagnetism) do not obey Newton\u2019s third law between particles \u2014 in real physics, the missing momentum is carried by the field.' },
        ...
        signaldelay: { title: 'Signal Delay', body: 'Finite-speed force propagation. Forces use each source particle\u2019s past position and velocity from its history buffer, solving the light-cone equation via Newton\u2013Raphson iteration. Creates realistic lag in distant interactions. Requires Relativity.' },
        spinorbit: { title: 'Spin\u2013Orbit Coupling', body: 'Transfers energy between translational and spin KE. EM: dE = \u2212\u03BC\u00B7(v\u00B7\u2207B_z)\u00B7dt where \u03BC = \u2155q\u03C9r\u00B2. GM: same pattern using angular momentum L = I\u03C9 and \u2207Bg_z. Requires Relativity and the relevant force toggle (Magnetic or Gravitomagnetic).' },
