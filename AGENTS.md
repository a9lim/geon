...
UI toggle hidden via `style="display:none"` — still activatable via presets (Roche limit preset). Two toggle-controlled mechanisms: tidal fragmentation (parent → SPAWN_COUNT children) and Roche lobe overflow (Eggleton 1983 mass transfer). The same CPU/GPU machinery also carries BH extremal charge shedding in black-hole mode, even when the hidden disintegration toggle is off. Known bugs:
