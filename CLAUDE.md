...
- **Periodic particle sync** (`_readbackParticleSync()`): Every 8 GPU frames, reads back `particleState` flags+position+mass+charge for all slots. `consumeParticleSync()` returns the data. Main loop's `_syncParticlesFromGPU()` uses this to: (1) update CPU particle positions/mass/charge from GPU state, (2) prune dead CPU particles whose GPU slot is no longer alive, (3) create CPU counterparts for GPU-only particles (merge survivors), (4) remove stale alias duplicates (two CPU particles sharing the same `_gpuIdx`). Uses `particleSyncStaging` buffer (persistent, `MAP_READ | COPY_DST`).
- CPU-side `particles[]` array maintained in parallel for sidebar UI, presets, hit testing, stats. `addParticle()` evicts stale CPU particles when reusing freed GPU slots.
